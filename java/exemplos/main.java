...
public class main {
	public static void main(String args[]) {
		mainPonto pOriginal, pAlias, pCopia;             //preparacao das variaveis copia de objetos
		
		pOriginal = new mainPonto((float)0.0,0.0f);      //(float)0.0 ou 0.0f; 0.0 eh double
		pOriginal.mostra();
		System.out.println(pOriginal.toString());
		
		pAlias = pOriginal;                              //copiando atraves de atribuica (ambos estão no mesmo endereço)
		pCopia = new mainPonto(pOriginal.x,pOriginal.y); //copiando atributo por atributo (são escritos em outro endereço)
